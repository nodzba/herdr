...
fn restore_tab(
    ...
    for id in &pane_ids {
        ...
        let saved_droid_session_id = saved_pane.and_then(|p| p.droid_session_id.clone());
        let saved_pi_session_file = saved_pane.and_then(|p| p.pi_session_file.clone());
        let saved_history =
        ...
        // Fork: bespoke droid/pi resume takes priority when a session was
        // persisted and its session file still exists on disk.
        let explicit_resume_argv: Option<Vec<String>> =
            if let Some(ref session_id) = saved_droid_session_id {
                ...
                if session_file.is_file() {
                    ...
                    tracing::warn!(
                        ...
                        "droid session file not found, using default restore"
                    );
                    None
                }
                ...
                if std::path::Path::new(session_file).is_file() {
                    ...
                    tracing::warn!(
                        ...
                        "pi session file not found, using default restore"
                    );
                    None
                }
            } else {
        ...
        let was_native_agent_restore =
            !was_imported && explicit_resume_argv.is_none() && startup.restore_plan.is_some();
        ...
        } else if let Some(argv) = explicit_resume_argv.as_ref() {
...
fn droid_session_jsonl_path(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    ...
    let dir_key = if let Some(rest) = cwd_str.strip_prefix('/') {
        format!("-{}", rest.replace('/', "-"))
    } else {
        cwd_str.replace('/', "-")
    };
